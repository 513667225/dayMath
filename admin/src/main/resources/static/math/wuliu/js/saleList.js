...
layui.use(['jquery', 'form', 'util', 'layer', 'upload','laydate'], function () {
  ...
  var end =   laydate.render({
    ...
    form.on('submit(serchBtn)', function (data) {
        ...
        $("#begin").val(data.field.begin);
        $("#end").val(data.field.end);
        return false
    })
    ...
    userTable = table.render({
        ...
        method: 'get',
        where:{
            begin: formatDay(new Date().setDate(new Date().getDate()-1)),
            end: formatDay(new Date().setDate(new Date().getDate()-1))
        },
        toolbar: '#toolbarDemo',
        cols: [[
            {type: 'checkbox'},
            {field: 'asin', width: 130, title: 'ASIN', sort: true},
            {field: 'productName', width:160, title: '品名', sort: true},
            {field: 'site', title: '站点', sort: true, align: 'center'},
            {field: 'user', title: '负责人', sort: true, align: 'center'},
            {field: 'times',  width: 130,title: '日期', sort: true, align: 'center',templet:function (data){
                    return data.times?formatDay(data.times):"";
                }},
            {field: 'salePrice', title: '预估销量', sort: true, align: 'center'},
            {field: 'beforeVal', title: '近期修改', sort: true, align: 'center'},
            {field: 'updateTime', width: 150,title: '修改时间', sort: true, align: 'center',templet:function (data){
                    return data.updateTime?formatTime(data.updateTime):"";
                }},
            {field: 'actualSale', title: '实际销量', sort: true, align: 'center'},
            {field: 'rate', title: '完成率', sort: true, align: 'center',templet:function (data){
                    return data.rate+"%";
                }},
            {field: 'fbaFirstSize', title: 'FBA期初库存', sort: true, align: 'center'},
            {field: 'estimateSize', title: '预估剩余库存', sort: true, align: 'center'},
            {field: 'actualSize', title: '实际剩余库存', sort: true, align: 'center'},
            {field: 'weekMonday', title: '周一库存', sort: true, align: 'center'},
            {field: 'ks', title: '可售天数', sort: true, align: 'center'},
            {field: 'inJson', title: '当日入库数量', sort: true, align: 'center',templet:function (data){
                    var inJson = data.inJson;
                    var sumCount=0;
                    if (inJson){
                        inJson = JSON.parse(inJson);
                        $.each(inJson,function (k,v) {
                            console.log(k,v)
                            sumCount +=v.quantityReceived*1;
                        })
                        return  sumCount;
                    }else {
                        return "";
            ...
            {field: 'inJson', width:250, title: '货件详情', sort: true, align: 'center',templet:function (data){
                    var inJson = data.inJson;
                    if (inJson){
                        inJson = JSON.parse(inJson);
                        var str = "";
                        $.each(inJson,function (k,v) {
                            // console.log(v)
                            //shop/shipmentId/count,
                            str+= v.name+"/"+v.shipmentId+"/"+v.quantityReceived+",";
                        })
                        return str;
                    }else {
                        return "";
            ...
            {field: 'retSum', title: '预估到货总量', sort: true, align: 'center'},
            {field: 'retSumStr', title: '预估到货详情', sort: true, align: 'center'},
    ...
    $("#begin").val(formatDay(new Date().setDate(new Date().getDate()-1)));
    $("#end").val(formatDay(new Date().setDate(new Date().getDate()-1)));
