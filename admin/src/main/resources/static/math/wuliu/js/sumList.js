...
layui.use(['jquery', 'form', 'util', 'layer', 'upload','laydate'], function () {
  ...
  var end =   laydate.render({
    ...
    userTable = table.render({
        elem: '#goods_table',
        id: 'taskId',
        url: '/sale/querySumSale',
        method: 'get',
        toolbar: '#toolbarDemo',
        cols: [[
            {type: 'checkbox'},
            {field: 'user', title: '负责人', sort: true, align: 'center'},
            {field: 'salePrice', title: '总销量', sort: true, align: 'center'},
            {field: 'actualSale', title: '总实际销量', sort: true, align: 'center'},
            {field: 'rate', title: '完成率', sort: true, align: 'center'},
    ...
    table.on('tool(goods_table)', function (obj) {
        ...
        if (obj.event === 'look') {
            var index = layer.open({
                ...
                area: ['1000px', '800px'],
                shadeClose: true,
                content: "sumAsinList.html?user=" + data.user+"&begin="+$("#begin").val()+"&end="+$("#end").val()
            });
